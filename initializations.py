...
class Distribution(nn.Module):
    def __init__(self, kind='uniform'):
        ...
        self.num_features = 2 * self.get_transforms(1).shape[0] + len(activation_ids)
    ...
    def get_weights(self, layer_dict, shape):
        dist = self.get_distribution(layer_dict['fan_in'], layer_dict['fan_out'], layer_dict['activation'])
    ...
    def get_distribution(self, fan_in, fan_out, activation_id):
        ...
        if self.kind == 'uniform':
            distribution = Uniform(params[0] - params[1], params[0] + params[1])
    ...
    def create_features(self, fan_in, fan_out, activation_id):
        activation_one_hot = self.one_hot(activation_ids[activation_id], len(activation_ids))
    ...
    def get_transforms(x):
        transforms = [x, x ** 0.5, x ** -0.5, 1/x, math.log(x)]
...
def initialize(target_model, distribution_model):
    ...
    for i, m in enumerate(target_model.children()):
        which_activation = {isinstance(m, activation): activation for activation in activation_ids.keys()}
        if any(list(which_activation.values())):
            try:
                ...
                m.weight, log_prob = distribution_model.get_weights(layer_dict, last_module.weight.shape)
...
def train_step(dist_model, reward, reward_mean, reward_std, log_probs, optim):
    optim.zero_grad()

    r_centered = (reward - reward_mean) / (len(log_probs) * reward_std)
    loss = 0
    for i in range(len(log_probs)):

        loss += -1 * r_centered * log_probs[i].sum()
...
if __name__ == "__main__":
    ...
    for i in range(1000):
        ...
        log_probs = initialize(target_model, dist_model)
        # print(log_probs)
        loss = train_step(dist_model, reward, reward_mean, reward_std, log_probs, optim)
